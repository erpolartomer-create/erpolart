...
class ModernTimer {
  ...
  getElapsedTime() {
    this.elapsedTime = (performance.now() - this._start) / 1000;
...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    class App {
      ...
      initPasses() {
        ...
        // SMAA can crash if assets aren't ready. We'll only add it if we have the assets.
        if (this.assets.smaa && this.assets.smaa.search && this.assets.smaa.area) {
      ...
      update(delta) {
        ...
        let fovChange = lerp(this.camera?.fov || 90, this.fovTarget, lerpPercentage);
        if (fovChange !== 0 && this.camera) {
      ...
      setSize(width, height, updateStyles) {
        if (width <= 0 || height <= 0 || !this.composer) {
          this.hasValidSize = false;
          return;
        }
      ...
      tick() {
        ...
        if (resizeRendererToDisplaySize(this.renderer, this.setSize)) {
          ...
          if (this.hasValidSize && this.camera) {
        ...
        if (this.hasValidSize) {
          const delta = this.clock ? this.clock.getDelta() : 0.0163;
    ...
    const roadMarkings_fragment = `
      ...
      float brokenLineWidth = uBrokenLinesWidthPercentage * laneWidth;
      float laneEmptySpace = 1.0 - uBrokenLinesLengthPercentage;

      // Broken lines between lanes
      float brokenLines = step(1.0 - uBrokenLinesWidthPercentage, fract(uv.x * uLanes)) * step(laneEmptySpace, fract(uv.y * 10.0));
      
      // Remove broken line on the very right edge to avoid overlap with shoulder
      brokenLines *= step(uv.x, 1.0 - laneWidth + 0.01);

      // Shoulder lines (continuous)
      float shoulderLines = step(1.0 - uShoulderLinesWidthPercentage, uv.x) + step(uv.x, uShoulderLinesWidthPercentage);
  ...
  return <div id="lights" ref={hyperspeed} className="select-none" style={{ WebkitTouchCallout: 'none', WebkitUserSelect: 'none', userSelect: 'none' }}></div>;
};
