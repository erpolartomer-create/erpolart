  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((p) => !p)}
        className="w-full flex items-center justify-between bg-surface border border-white/8 rounded-2xl px-6 py-4 text-white text-sm font-medium focus:border-amber-500/50 focus:ring-4 focus:ring-amber-500/5 outline-none transition-all shadow-sm cursor-pointer hover:border-white/20"
      >
          ...
          >
            {options.map((opt) => (
              ...
              >
                ...
                {opt.value === value && <Check size={13} className="text-amber-500 shrink-0" />}
              </button>
            ))}
          </motion.div>
        )}
...
const SaaSContactSection = () => {
  ...
  const inputClass = 'w-full bg-surface border border-white/8 rounded-2xl px-6 py-4 text-white text-sm font-medium placeholder-gray-600 focus:border-amber-500/50 focus:ring-4 focus:ring-amber-500/5 outline-none transition-all shadow-sm';
  ...
  return (
    <section id="saas-contact" className="py-16 md:py-28 relative">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-[400px] bg-amber-500/5 blur-[160px] rounded-full" />
      </div>
      ...
      <div className="max-w-3xl mx-auto px-6 relative z-10">
        {!isSubmitted ? (
              ...
              <div className="bg-surface/60 backdrop-blur-2xl border border-white/8 rounded-[2.5rem] p-8 md:p-12">
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="group relative w-full py-5 rounded-2xl bg-gradient-to-r from-amber-400 via-amber-500 to-amber-600 text-black font-black text-xs uppercase tracking-[0.3em] overflow-hidden transition-all duration-300 hover:shadow-[0_20px_40px_rgba(245,158,11,0.3)] hover:scale-[1.01] active:scale-[0.98] disabled:opacity-70 disabled:pointer-events-none"
                  >
          ...
          <motion.div
            ...
            className="bg-surface/60 backdrop-blur-2xl border border-amber-500/20 rounded-[2.5rem] p-12 md:p-20 text-center"
          >
            <div className="w-20 h-20 rounded-full bg-amber-500/10 flex items-center justify-center mx-auto mb-8 shadow-[0_0_50px_rgba(245,158,11,0.15)]">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2.5 rounded-full bg-amber-500/8 border border-amber-500/20 mb-10">
              <Mail size={13} className="text-amber-500 shrink-0" />
              <span className="text-gray-400 text-xs">{t('contact.success.emailNote')}</span>
              <span className="text-amber-400 text-xs font-bold">{formData.email}</span>
            </div>
            <button
              onClick={() => setIsSubmitted(false)}
              className="text-amber-500 font-black text-xs uppercase tracking-[0.2em] border-b-2 border-amber-500/30 pb-1 hover:border-amber-500 transition-all"
            >
...
export default SaaSContactSection;
